...
- **日期：** 2026-09-10
- **状态：** 已交付（P1b 全部任务完成；终态 289/289/0，selfcheck 全行通过含 learned 行）
...
- [x] Step 1: 失败测试——http URL 行解析出 `{name, url, transport:'http'}`；URL 行 + `transport:sse` → sse；普通命令行缺省 stdio；`transport:bogus` 跳过；缺 name/url/command 跳过
- [x] Step 2: `npm run build && node --test dist/config.mcp.transport.test.js` 确认红
- [x] Step 3: 最小实现 types 增量 + 解析分支
- [x] Step 4: 确认绿 + 全量不回归
- [x] Step 5: 提交 `feat(mcp): McpServerConfig transport/url 字段与 parseMcpServers 传输感知解析`
...
- [x] Step 1: 失败测试——不可达 http URL → `MCP_CONNECT_FAILED`；`transport` 缺省行为与既有 stdio 用例一致（回归由全量套件保障）
- [x] Step 2: 确认红
- [x] Step 3: 最小实现三分支工厂（switch 收敛一处）
- [x] Step 4: 确认绿 + 全量不回归
- [x] Step 5: 提交 `feat(mcp): McpHost 传输工厂——streamable http / sse 分支接入既有握手与闸门链`
...
- [x] Step 1: 冒烟脚本自测（curl/原始 fetch 往返 initialize + tools/list）
- [x] Step 2: 确认协议帧与 SDK 客户端期望一致
- [x] Step 3: 实现并跑通两种模式冒烟
- [x] Step 4: 提交 `test(mcp): 零依赖 HTTP/SSE mock server（streamable http 与 sse 双模式）`
...
- [x] Step 1: 失败测试——http 与 sse 两用例（注册链 + 安全链调用 + mask）
- [x] Step 2: 确认红
- [x] Step 3: 确认绿（H2/H3 实现已就位，本任务以集成验证为主）
- [x] Step 4: spec §3.1 状态同步（HTTP/SSE 传输已交付；OAuth 仍留位）
- [x] Step 5: 提交 `test(mcp): HTTP/SSE 端到端集成（注册→安全链→mask 出口）+ spec §3.1 状态同步`
...
- [x] Step 1: 失败测试——写入后 `parseSkillFrontmatter` 可读回；同名 goal 二次 settle 得 `-2`；中文/特殊字符 slug 确定性；超 50 删最旧
- [x] Step 2: 确认红
- [x] Step 3: 最小实现
- [x] Step 4: 确认绿 + 全量不回归
- [x] Step 5: 提交 `feat(skills): LearnedSkillStore——成功任务沉淀为学习技能模板（.data/skills，FIFO 50）`
...
- [x] Step 1: 失败测试——stub 模型 done 路径触发 settle 一次且产物可被 frontmatter 解析；失败路径零触发；settle 抛错不影响 RunResult
- [x] Step 2: 确认红
- [x] Step 3: 最小实现钩子与装配
- [x] Step 4: 确认绿 + 全量不回归
- [x] Step 5: 提交 `feat(reactor): 成功任务沉淀钩子——done 路径一次 settle，失败零触发，装配缺省开启`
...
- [x] Step 1: 失败测试——学习技能可 resolve；用户同 id 遮蔽学习产物；无 .data/skills 目录零影响；selfcheck 行存在
- [x] Step 2: 确认红
- [x] Step 3: 最小实现合并与 selfcheck 行
- [x] Step 4: 确认绿 + 全量不回归
- [x] Step 5: 提交 `feat(skills): SkillsFacade 双根合并（用户优先）+ selfcheck 学习技能行`
...
- [x] Step 1: 全量门禁——build 0 / 全量测试 0 失败 / selfcheck 通过（记录终态基线）
- [x] Step 2: spec §6-R5 写入定案（LearnedSkillStore + Reactor 钩子 + 双根合并三件套；自动技能筛选/AI 语义聚类仍留阶段五）
- [x] Step 3: ROADMAP 阶段四条目同步
- [x] Step 4: 提交 `docs(phase4-p1b): G1——spec §6-R5 技能沉淀闭环定案 + ROADMAP 同步 + 终验门禁记录`
