...
- **展开**：`Ctrl+O` 把单行展开为多行面板（每节点一行：状态 + tokens + 耗时），再按收起；展开态不持久化。**不复用 `⏎`（已用于发送）与 `Tab`（`App.tsx` 已有绑定）**。节点详情仍按 §5.3 推正文 `step` 行，进度条只承载「当前」。**键位调和（2026-09-11）**：`Ctrl+O` 已划归《2026-09-11-tui-display-and-commands-design.md》的会话全文视图；本文阶段 C 落地时，FlowStrip 的多行展开面板并入该全文视图承载（一个覆盖层同时承载过程与流程），不再单开覆盖层。
