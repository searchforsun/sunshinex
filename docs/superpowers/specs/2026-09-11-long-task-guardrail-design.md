...
- **展开**：`Ctrl+O` 把单行展开为多行面板（每节点一行：状态 + tokens + 耗时），再按收起；展开态不持久化。**不复用 `⏎`（已用于发送）与 `Tab`（`App.tsx` 已有绑定）**。节点详情仍按 §5.3 推正文 `step` 行，进度条只承载「当前」。**键位调和（2026-09-11，v2）**：`Ctrl+O` 已划归《2026-09-11-tui-display-and-commands-design.md》的**事件面板**（钉底单行实时区原地展开，运行中 / 空闲均可用）；本文阶段 C 落地时，FlowStrip 的多行展开面板并入该面板承载，不再单开覆盖层。
...
- 展示（阶段 C）：进度条活跃集合渲染（并发层多节点同显）、窄终端尾部截断且活跃节点保留、`Ctrl+O` 展开 / 收起切换。
...
按四个可独立验收的段落推进：**A 护栏内建（1–3）** → **B 接线与可见性（4–7）** → **B+ 专有模式（8）** → **C 运行态展示（9）**。A 完成后底座即可独立验证（Reactor 三种 `stopReason`）；B 依赖 A；B+ 与 C 复用同一事件源，C 可延后而 A / B 仍完整可用。
