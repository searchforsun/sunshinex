...
目标：指针概念整体退出、最近会话解析统一走会话档 mtime、补 `--resume` 启动旗标对标 CC、DATA_DIR 收口为开发测试专用口。
...
| D2 | 新增 TUI 启动旗标 `--resume`：启动交互终端并立即弹出会话选择卡（复用 askUser/OptionSelector 通道与 `/resume` 卡片形态：↑↓ 移动、Enter 恢复、Esc 取消、数字快选）；Esc=放弃恢复以新会话继续；列表空=回执「无可恢复的会话」后进新会话；目录沿既有顶层形态指定（`sunshinex [dir] --resume` / `--workdir=`） | 对标 CC `--resume` 无参选择器；旗标形态与 `--continue` 对仗、命令面零新增 |
| D3 | `--continue` 用户语义不变（续接最近会话），实现切到 mtime 口径；`--resume` 与 `--continue` 同传 fail-fast（前者弹选择卡、后者直取最近，择一语义）；`--resume` 与 `--worktree` 互斥（沿 `--continue` 先例：恢复会话的活动 root 属会话自身事实） | 恢复语义单一、边界清晰 |
...
| `src/tui/session.ts` | `resumeLatest()`（`--continue`）改 `listSessions(dataDir)[0]`；新增 `resumeFlow()` 选择卡单点（/resume 与启动选择卡共用、候选口径一致），选定走既有 `restoreFromSession` 恢复路径；opts 增 `resumePicker` |
| `src/tui/entry.ts` | `--resume` 旗标解析、与 `--continue` 同传 fail-fast、`resumePicker` 透传控制器 |
| `src/cli/worktree-launch.ts` | `--resume` 与 `--worktree` 互斥校验（沿 `--continue` 先例同点位） |
| `src/cli/index.ts` | USAGE 双语 flags 行补 `--resume` |
...
3. `--resume` 选择卡：列表渲染（排除当前/空档）、Enter 恢复选定档、Esc 新会话、空列表回执；`--resume`+`--continue` 与 `--resume`+`--worktree` 同传均 fail-fast。
...
| 4 | `--resume` + `--continue`、`--resume` + `--worktree` 同传均报互斥 |
