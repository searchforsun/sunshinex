...
自审：2026-09-20 修正三处——分支点边界语义（锚点行不进新档、upToLine=锚点行号−1）、代码回退收集面（含当轮写入）、upToLine=1（仅 header 空档）合法化。
复审：2026-09-20 §6.2 收集面由「血缘上溯」修正为「单文件收集」——逐字节前缀复制下锚点前历史（含血缘祖先事件）已物理内含于当前档，无需上溯；祖先档分支点之后的事件属平行时间线，纳入将造成重复回退（正确性缺陷，非仅简化）。
...
分支点语义=第 k 轮的**起点**（与对话回退同界），恢复目标=第 k 轮开场时的文件状态，**含第 k 轮自身写入的 pre-image**（其清单恰登记在该轮 user 事件上）。收集面=**单一档案**：分档为逐字节前缀复制，目标锚点之前的全部历史（含血缘祖先的事件）已物理内含于当前档，无需上溯 forkedFrom；祖先档分支点之后的事件属平行时间线，纳入即重复回退，禁止。算法：在当前会话档内收集行号 ≥ k 的 user 事件 files 清单（≥ 含锚点轮自身），每文件取**行号最早**一条（行号序=时间序，最早即最接近锚点时点）：`deleted: true` → 删除该文件；否则以 hash 取 blob 内容写回。无清单的文件不动。/rewind 即时回退直接以分支前源档为收集档（与经新档 header.forkedFrom 解析同源）；对已分叉会话更早锚点的再回退，在其自身档内同算法收集。
...
8. 跨层前缀回退：fork 出的会话（内含源档前缀与 files 清单）内对源档时间线的锚点执行 rewind 代码回退，仅凭自身档收集正确（每文件恰取最早 pre-image；祖先平行时间线事件不参与）。
