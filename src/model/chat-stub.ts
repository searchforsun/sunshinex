...
import { parseScriptStep } from './adapter';
import type { ChatRequest, ChatResult } from '../types';
import type { UsageHooks } from './adapter';

/** 文本应答回调：入参（消息视图串接文本、用量钩子、中断信号），返回 JSON DSL 文本（单工具/并行/done）；同步/异步皆可 */
 ...
 *  应答文本经 JSON DSL 转译为结构化出牌。仅测试面消费，产品代码走适配器本体的 chat 实现 */
export function textReplyToChatFace(reply: TextReplyFn): (req: ChatRequest, hooks?: UsageHooks) => Promise<ChatResult> {
  return (req, hooks) => {
    const prompt = req.messages.map((m) => m.content).join('\n');
    return Promise.resolve(reply(prompt, hooks, req.signal)).then((text) => parseScriptStep(text));
  };
}
