...
/**
 * 语义键 → SUNSHINEX_* 环境槽映射表（规格 §6 全表，全仓唯一权威）。
 * - 凡 *API_KEY 一律不进本表（D6）：密钥只走 env 透传块或环境变量，设置与凭据分离。
 * - SUNSHINEX_SHELL 平台逃生口不语义化，仅 env 块/环境变量承载。
 * - language 是唯一新增槽（D9）：用户级个人偏好，此前没有 env 槽，其余键均为既有槽位的语义化。
 */
 ...
 * - 文件缺失返回 null 静默跳过（对齐 loadEnv 现状：没配就是没配，不是错误）
...
/**
 * 装载：parse → flatten → 只填 process.env 未定义键（loadEnv 同款语义）。
 * 已导出环境变量与先装层级（项目 .env）不被覆盖——装载顺序即优先级（D2）。
