...
test('App：Tab 切换历史展开模式——运行中可切、回调触发、现场回写、可逆', async () => {
  ...
  try {
    ...
    const retain = { buffer: '', cursor: 0, expandAll: false, latestFull: false, spawnExpanded: [], browseMode: false, browseCursor: 0, history: [], histIdx: -1 };
  ...
  try {
    ...
    const { allOutput, unmount } = render(
      <App
        controller={ctrl}
        banner={{ version: '1.0.0', model: 'm', root: tmp }}
        retain={{ buffer: '', cursor: 0, expandAll: true, latestFull: true, spawnExpanded: [], browseMode: false, browseCursor: 0, history: [], histIdx: -1 }}
      />,
    );
    await sleep(150);
    const all = allOutput();
