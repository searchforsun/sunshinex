...
- **消息形态不变量**：模型上下文以消息视图为源（`buildMessages` 派生单点，链/快照/压缩块/技能块 → ChatMessage 序列），文本信封协议已退役；tools 清单为请求级字段（`tools`），不进提示词文本、前缀零占用；system 双消息（稳定段 + 会话冻结段）逐字节冻结语义同前置段
