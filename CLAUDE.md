...
> 包管理器统一 pnpm：版本由 `packageManager` 字段钉定（corepack 启用后直接使用 `pnpm`）；`.npmrc` 已将 store 固定在仓内 `.pnpm-store`，沙箱等 HOME 不可写环境开箱即用（部署口径见 `docs/PLATFORM.md`）。
...
- **核心契约零兼容**：模型核心契约（原生 function calling 等 API 层强制的能力）不支持即换模型/换端点，禁止在提示词层做兼容适配——靠提示词约束模型输出分布的兼容是开集，补丁修不完（文本信封协议已退役为教训）。允许保留的容错仅限两类：①wire 层差异（输入形态闭集 + 每种形态有确定归一规则 + 未知形态可判定并安全回退，如流式 tool_calls 分片重组、finish_reason 归一）；②运行时自愈与可选增强降级（上下文超限反应式压缩、reasoning_effort 探测降级——缺了只损增强不损核心契约）。新增任何兼容/兜底须先过此判据：需要预测「模型下一步会输出什么」的补丁一律不做
...
- 技能标准形态：`{根}/skills/{id}/SKILL.md`（SKILL.md 优先、skill.md 兜底保三平台装载一致），含 frontmatter（name/description/version）与正文。**只装载标准形态，异构形态零兼容**——`rules/*.mdc`、裸 `AGENTS.md`、`commands/*.md` 一律不装载（有防回归钉子用例）。
...
- 端点侧行为本产品不可控：真实端点以探针口径观测（两轮前缀连续请求，第二轮 `cached_tokens` 明显大于 0 即命中）；端点未启用前缀缓存的残余缺口归端点侧，不计入产品回归目标。
...
- **工程约束（编码时强制）**：路径一律 `path.join` / `path.resolve` / `path.relative`，禁止手拼分隔符；**路径子树包含判定一律走 `isWithin`（`src/paths.ts`，叶子模块避免模块环），禁止手写 `startsWith(root + path.sep)`**——只归一一侧即前缀恒失配、静默全拒，曾为此付出代价（裁决记录见 `memory/paths.ts` 头注）；子进程执行收敛在 `ProcessSandbox` 单点，平台分支只允许出现在该文件（脚本层 `.cmd` 分派见 `docs/PLATFORM.md`）；pnpm scripts 保持零 shell 语法依赖（仅 `&&`）；glob 匹配与产物统一 `/` 分隔——`listFiles` 对 `path.relative` 结果先归一化再匹配（Windows 反斜杠转 `/`，POSIX 为 no-op）。
...
- **已知差异（如实登记，不虚构兼容）**：`exec` shell 由 `resolveShell()` 按序解析——`SUNSHINEX_SHELL` 显式覆盖（须 POSIX 兼容，配 `-c` 调用）→ Windows 探测 Git Bash → 无 Git Bash 时探测 PowerShell（pwsh 各候选整体先于 powershell.exe，`-NoProfile -Command`，对齐 Claude Code native Windows 口径）→ 末位 `ComSpec` 兜底（不崩，sh 语义命令不保证可用）→ POSIX `/bin/sh`。决议产物带来源标签（`override`/`git-bash`/`powershell`/`comspec`/`posix`），`selfcheck` 的 `shell :` 行显式上屏。Git Bash/PowerShell 候选序、WSL 启动器排除、便携版教训等机制细节见 `docs/PLATFORM.md`。仓库文本为 LF（由机器强制承载，见上）。
- **已知差异 · Windows 超时与进程树（接受并登记，不加代码）**：超时回收依赖 `killSignal`（POSIX `SIGTERM` 只及直接子进程、Windows `TerminateProcess` 不级联子进程树），超时后可能有孤儿进程残留。接受理由：主链唯一子进程调用即 `exec` 且多为短命进程；彻底收口需 `taskkill /T /F` / Job Object / POSIX 进程组，属跨平台进程管理独立议题，须单独立项论证，不以顺手补丁处置。
- **平台相关改动纪律**：新增任何平台相关行为（路径、进程、信号、权限）须在本节登记差异与结论，并同步复核 README 平台支持矩阵与部署指引。
- **测试不得编入宿主 shell 方言**：被测命令形态须对 shell 中立——不以 `ls`/`cat`/内联 `node -e "…"` 作断言前提（Windows 无 Git Bash 时前者不可用、后者被当字面量、断言恒真失去判别力），一律以脚本文件（`node script.js`）或跨 shell 命令承载；shell 语义用例集中在 `security/sandbox.test.ts`（平台分支唯一落点），其余测试只断言工具链行为。
