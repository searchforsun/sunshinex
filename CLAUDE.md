...
推理成本与响应时延是 agent 产品的核心竞争指标，前缀缓存（供应商侧自动 prefix cache，自开头逐字节匹配）是最有效的杠杆——不管产品什么形态（CLI/TUI/GUI、直执行/plan/工作流），能力建设都必须建立在「相邻请求 token 前缀命中率最大化」之上。对标基线：Claude Code 极致前缀缓存方案——系统提示词与工具清单整场冻结、对话只尾部追加、历史仅在压缩点重写、稳定段尽可能长。

核心不变量（违反即回归；任何新能力先回答「落在提示词哪个位置、击穿多长前缀」）：

- **提示词单一来源**：任务执行轮的提示词只由 `Reactor.buildPrompt()` 产出（身份、输出约定、phase 约定、工具清单、JSON 协议、上下文）；loop/graph 的角色框定与任务文本复用该主链（判据仲裁等独立一次性调用除外，但其内部拼装同样禁止时变字段）；各层禁止绕开主链自行拼提示词。
- **稳定段确定性**：history 段之前的任何内容须同任务内逐字节稳定——不得含时间戳/随机值/高频时变字段（git 状态、计数器等）；工具清单按名排序（注册序不得泄入提示词）；工作目录等环境事实为会话级常量；每步观察只经 history 尾部追加承载，禁写回 memory 等前置段（历史教训：每步观察双写 memory 曾把命中率压到 41.1%）。
- **history 只追加，压缩是唯一合法重写点**：相邻步提示词只允许尾部增长；任何对 history 的改写/缩减、注入块的消失都构成击穿——唯一合法的全量重写是上下文压缩（预算驱动的必然，带滞回节流）；技能正文等一次性注入应置于提示词尾部（history 之后），使其消失时击穿面最小，禁止把断点放在稳定段前部（现有技能首帧注入位于上下文段头部、正属此模式，待修）。
- **重算事件应少且收敛**：跨任务（goal 变化）、上下文压缩、模型档位切换是三处公认的断点重算；模型档位是用户级会话参数（`--tier` / TUI `/model` / `SUNSHINEX_TIER`，对标 Claude Code 的模型选择）：整场恒定、不进提示词、模型无自调通道（`reply.tier` 协商已摘除）、系统不做逐步重估与自动换档；按档绑定模型时换档即换模型，属用户显式触发的重算事件。提示词内不存在每步可变行——唯一合法的相邻步差异是 history 尾部追加（与压缩重写）。

验证与回归（改动强制项）：

- 提示词组装面改动必须附带「相邻步前缀稳定」回归用例（断言相邻步公共前缀占比 / 首个差异点落在尾部新增段），并跑 `pnpm build` + 全量测试 + `pnpm selfcheck`。
