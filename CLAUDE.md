...
- **提示词单一来源**：任务执行轮的提示词只由 `Reactor.buildPrompt()` 产出（身份、输出约定、phase 约定、工具清单、JSON 协议、上下文、档位行）；loop/graph 的角色框定与任务文本复用该主链（判据仲裁等独立一次性调用除外，但其内部拼装同样禁止时变字段）；各层禁止绕开主链自行拼提示词。
...
- **重算事件应少且收敛**：跨任务（goal 变化）、上下文压缩、算力档位切换（绑定不同模型时）是三处公认的断点重算；档位路由应避免逐步震荡（升档宜棘轮保持）；档位行等可变行恒居提示词最尾（变化至多击穿末行）。
