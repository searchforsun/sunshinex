...
- **消费方定归属**：只有用户消费的串（审批卡、状态栏、命令帮助）走 `t()` 双语；只要进模型上下文（观察、错误、工具结果、链行），即使同时上屏展示，也一律英文单语——不为同一串维护两份语料。
- **机械校验**：提示词组装面（`buildPrompt`、工具 description、观察产出点、生成类 goal）不得出现非 ASCII 字符（项目专名如 `SUNSHINE.md` 除外），以审计用例断言；`t()` 仅允许出现在界面呈现层。
